...
const worker = new Worker(
  ...
  async (job) => {
    ...
    // Update payment
    if (success) {
      await db.query("UPDATE payments SET status='success', updated_at=NOW() WHERE id=$1", [paymentId]);
      console.log(`✅ Payment ${paymentId} succeeded`);
    } else {
      await db.query(
        "UPDATE payments SET status='failed', error_code='PAYMENT_FAILED', error_description='Payment processing failed', updated_at=NOW() WHERE id=$1",
        [paymentId]
      );
    ...
    await webhookQueue.add("deliver", {
      merchantId: payment.merchant_id,
      event: success ? "payment.success" : "payment.failed",
      payload: { payment: { ...payment, status: success ? "success" : "failed" } },
    });
  },
  { connection: createRedisConnection() }
);
